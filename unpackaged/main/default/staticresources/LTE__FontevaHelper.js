(function (root, factory) {
    ...
    if (typeof define === 'function' && define.amd) {
        ...
        AuraPromise.prototype.finally = function (onFinally) {
            // polyfill for IE11 .finally()
            const IE11PolyfillFinally = function(callback) {
                // ensure that the callback is a function, else create an empty function
                callback = callback instanceof Function ? callback: function(){}

                const lastPromiseInChain = function() {
                    // get a reference to the last Promise/thenable right before having called .finally()
                    return this;
                }.bind(this) //ensure the original calling context is bound

                const decoratedCallback = function() {
                    // run the callback passed into .finally()
                    // then return the reference to the last Promise/thenable right before having called .finally() to preserve the resolved value or rejected reason
                    return AuraPromise.resolve(callback()).then(lastPromiseInChain);
                }

                // run .finally() as a .then() that invokes the decorated callback for both its resolve and reject functions
                // and return a Promise
                return this.then(decoratedCallback,decoratedCallback);
            };

            // if .finally() does not natively exist, then polyfill it using the above IE11 polyfill
            if(!Promise.prototype.hasOwnProperty('finally')) {
                Promise.prototype.finally = IE11PolyfillFinally;
            }
        ...
        if (!Array.prototype.flatMap) { // stupid edge doesn't even have flatMap
    ...
    return {
        ...
        ROUTINGPATH: {
            ...
            EVENTREG: {
                ...
                WAITLISTCONFIRMATION: '/reg/waitlistConfirmation'
            },
            hashesEqual: function(hashA, hashB) {
                return this.stripHash(hashA) === this.stripHash(hashB);
            },
            stripHash: function(hashString) {
                return hashString === undefined ? hashString : (hashString[0] === '#' ? hashString.substring(1) : hashString);
            }
        },
        notEmpty: function (value) {
            return !$A.util.isEmpty(value);
        },
        formatLabel: function(targetStr, argumentList) {
        ...
        getUrlPostHash: function (paramName) {
            var sPageURL = decodeURIComponent(FH.ROUTINGPATH.getCurrentWindowHash()),
        ...
        showErrorMessage: function (message, type, title, cmp) {
            ...
            if (toastEvent) {
                ...
                const cmps = cmp.find('toast') || cmp.find('toastMessages');
                const targetCmp = cmps instanceof Array
                    ? cmps.length > 0 ? cmps[0] : null
                    : cmps;
                if (targetCmp) {
                    targetCmp.showMessage(title, message, false, type);
                }
                else {
        ...
        getSalesOrderId: function (orgId) {
            orgId = orgId || FontevaHelper.getCacheItem('organizationId');
        ...
        isPromise: function(promise) {
            return promise instanceof Promise || (promise.innerPromise && promise.innerPromise instanceof Promise);
        },
        nullPromise: function (retVal) {
            return new AuraPromise(function (resolve) {
                resolve(retVal);
            });
        },
        getDataSet: function (cmp, evt) {
            let cmpDataset = {};
            if (evt.currentTarget && evt.currentTarget.dataset) {
                return evt.currentTarget.dataset;
            }
            let datasetObj = evt.getSource().getElements()[0];
            if (evt.getSource() && datasetObj) {
                cmpDataset = Object.assign(cmpDataset, JSON.parse(JSON.stringify(datasetObj.dataset)));
                // dataset values might be stored individually as 'dtaset-' attribute (PD-25852)
                for (let attr in datasetObj) {
                    let dtasetval = datasetObj[attr];
                    if ( dtasetval && !cmpDataset[attr] && attr.startsWith('dtaset') ) {
                        cmpDataset[attr.substring(6)] = dtasetval;
                    }
                }
            }
            if (evt.target && evt.target.dataset) {
                cmpDataset = Object.assign(cmpDataset, evt.target.dataset);
            }
            return cmpDataset;
        },
        ...
        createComponent: function (cmpName, params) {
            ...
            return new AuraPromise(function (resolve, reject) {
                $A.createComponent(cmpName, params, function (cmp, status, message) {
                    ...
                    if (verbose) console.log('FontevaHelper created: ' + cmpName + '; ' + cmp.getGlobalId());
                    resolve(cmp);
                });
            })
        },
        ...
        validateAuraInputs: function (inputFields) {
            let validated = true;
            inputFields = inputFields instanceof Array ? inputFields:[inputFields]

            inputFields.forEach(function(inputField) {
                // if at least 1 field is invalid flag form as invalid
        ...
        getLocalStorage: function(localStorageKey, defaultValue) {
            let localStorageValue;
            let LSSIndex = Object.keys(localStorage).find(k => k.includes('LSSIndex'));
            if (LSSIndex) {
                localStorageValue = localStorage.getItem(JSON.parse(localStorage.getItem(LSSIndex))[localStorageKey]);
            } else {
                localStorageValue = localStorage.getItem(localStorageKey) || defaultValue;
            }
            return localStorageValue;
        },
        getCurrentCurrencyIsoCode: function() {
            return this.getLocalStorage('currentCurrencyIsoCode', 'USD');
        },
        getOverrideCurrencyIsoCode: function() {
            return this.getLocalStorage('overrideCurrencyIsoCode', '');
        },
        Login: {
                ...
                fields.forEach(function(field) {
                    ...
                    FH.createComponent('markup://Framework:InputFields', {
                        ...
                        .then(function(newCmp) {
                            ...
                            if (field.fieldType.toLowerCase().includes('picklist')) {
                                newCmp.setSelectOptions(field.selectOptions);
                            }
        ...
        Events: {
             ...
             * @param soLine
             * @param label
             * @param isDisabled - used to disable the input for guests of invited attendees
             * @param contactIdsOnThisOrder
             * @param lookupIndex
             * @param onCreateCallback
             * @returns {Promise<T | never>}
             */
            initializeContactLookupField: function(cmp, soLine, label, isDisabled,addNewPersonLabel, contactIdsOnThisOrder, lookupIndex, onCreateCallback) {
                        ...
                        render: {
                            option: function(item, escape) {
                                ...
                                return '<div class="slds-grid">' +
                                    ...
                                    '<img alt="" src="' + $A.get('$Resource.Framework__SLDS_Icons') +
                ...
                if (allowCreate) {
                    otherAttributes.otherMethods.render.option_create = function(data, escape) {
                    	var newPersonMessage = addNewPersonLabel.replace(/({\d})/gi, escape(data.input));
                        if (!$A.util.isEmpty(newPersonMessage)) {
                        	return '<div class="slds-grid slds-grid_vertical create" data-selectable="true">' +
                            '<div class="slds-m-bottom_xx-small">' + newPersonMessage + '</div>' +
                            '</div>';
                        }
                        return '<div class="slds-grid slds-grid_vertical create" data-selectable="true">' +
                ...
                // InvitedAttendeeObj will be set only for
                // 1) Primary Invitee salesOrderLine or waitlist or groupTicketPrimaryAssignment.
                // 2) Event can be private or public.
                if (cmp.get('v.invitedAttendeeObj.id')) {
                    cmp.set('v.primaryRegistrationInvitation', true);

                    /* @todo we should use either input field disabled attr or otherAttributes disabled property, we can't set both attrs at the same time. */
                    disableInput = true;
                    otherAttributes.disabled = true;
                }
                ...
                return FH.createComponent('markup://Framework:InputFields', {
                    ...
                    if (divComponent) {
                        const divBody = divComponent.get("v.body");
                        divBody.push(newCmp);
            ...
            navToSecondRegistrationStep: function(cmp, isRSVP, eventObj, loginRedirect) {
                ...
                // changed to catch the case where some tickets have forms
                // need to fill out the forms event if `disableAttendeeAssignment` is true
                let skipAssignment = eventObj.isSingleTicketEvent ||
                    eventObj.disableAttendeeAssignment &&
                    salesOrder.lines.filter(function(l) { return !l.assignmentComplete; }).length === 0;

                const navToCheckout = function() {
                    if (eventObj.freeEvent) {
                        window.location.hash = regPath.CONFIRMATION;
                    }
                    else if (eventObj.registrationStyle === 'Community Event') {
                        ...
                        FH.waitForDataPromise('currentSite')
                            .then(function(site) {
                                window.onbeforeunload = null;
                                window.onunload = null;
                                window.location = site.siteUrl + '/store#/store/cart/' + (salesOrder.salesOrderId || salesOrder.id);
                            })
                    }
                    else {
                ...
                if (cmp.get('v.isEdit')) {
                    window.location.hash = windowsLocationHash;
                }
                else if (salesOrder && salesOrder.waitlistEntries && salesOrder.waitlistEntries.length > 0 && salesOrder.lines && salesOrder.lines.length === 0) { // valid for all event types
                    window.location.hash = regPath.CHECKOUT; // User needs to confirm the waitlist on checkout page
                }
                else if (!skipAssignment && _.indexOf([regPath.TICKETASSIGNMENT, regPath.TICKETS, regPath.OVERVIEW, regPath.NEWATTENDEE], windowsLocationHash) === -1) {
                    window.location.hash = regPath.TICKETASSIGNMENT;
                }
                else if (!skipAssignment && loginRedirect) { //special case when returning from login
                    window.location.hash = regPath.TICKETASSIGNMENT;
                }
                else if (salesOrder.hasTicketsWithSeating) {
                    window.location.hash = regPath.SEATING;
                }
                else if (eventObj.sessionsEnabled && salesOrder.hasAtleastOneActiveAgenda) {
                ...
                else if ([regPath.TICKETASSIGNMENT, regPath.TICKETS, regPath.NEWATTENDEE, regPath.SELECTATTENDEE].includes(windowsLocationHash)) {
                    navToCheckout();
                }
                else if (isRSVP) {
            ...
            generateRedirectUrl: function (component, baseUrl, hash) {
                if (baseUrl.includes('?')) {
                    baseUrl += '&';
                }
                else {
                ...
                return baseUrl + 'startURL=' + encodeURIComponent(window.location.pathname + window.location.search + hash);
            },
        salesOrderHelpers: {
            ...
            removeErrorMessageFromRedirectUrl(redirectUrl) {
                // need to remove the error message from url for failed transaction before we re process the same transaction
                if (redirectUrl.includes('message') && !$A.util.isEmpty(FontevaHelper.getUrlParameter('message'))) {
                    const tempURL = redirectUrl.split('#');
                    redirectUrl = new URL(tempURL[0]);
                    redirectUrl = redirectUrl.origin + redirectUrl.pathname;
                    let index = 0;
                    Object.entries(FontevaHelper.getUrlVars()).forEach(([key,value]) => {
                        if(key !== 'message'){
                            let operator = index > 0 ? '&' : '?';
                            redirectUrl += `${operator}${key}=${value}`;
                            index++;
                        }
                    });
