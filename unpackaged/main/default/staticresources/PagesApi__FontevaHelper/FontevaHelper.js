(function(root, factory) {
    ...
    if (typeof define === 'function' && define.amd) {
        ...
        if (!root.FontevaHelper) { //don't stomp LTE fontevaHelper if that's already loaded
    ...
    return {
        ...
        cacheItem: function(key, value, ttl) {
            ...
            sessionStorage.setItem('fonteva.' + key, JSON.stringify({
                cachedObj: value,
                insertDate: new Date(),
                ttl: ttl
            }));
        ...
        clearCacheItem: function(key) {
            sessionStorage.removeItem('fonteva.' + key);
        },
        getCacheItem: function(key) {
            var cachedObj = sessionStorage.getItem('fonteva.' + key);
        ...
        flushExpired: function() {
            for (var key in sessionStorage) {
                if (key.indexOf('fonteva') > -1 && (new Date() - Date.parse(cachedObj.insertDate)) > cachedObj.ttl) {
        ...
        showMainData : function() {
            document.getElementById('mainLoader').classList.add('slds-hide');
            document.getElementById('mainContent').classList.remove('slds-hide');
        },
